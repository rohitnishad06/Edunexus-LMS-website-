...
function EditCourses() {
  ...
  const getCourseById = async () => {
    try{
      const result = await axios.get(serverUrl + `/api/course/getcourses/${courseId}`, {withCredentials: true});
  ...
  return (
    <div className='max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md'>
      ...
      <FaArrowLeft className='top-[20px] md:top-[20px] absolute left-[0] md:left-[2%] w-[22px] h-[22px] cursor-pointer' onClick={() => navigate("/courses")} />
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <form className='space-y-6' onSubmit={(e)=>e.preventDefault()}>
          ...
          <div>
            ...
            <textarea id="des" className='w-full border px-4 py-2 rounded-md h-24 resize-none' placeholder='course Description' onChange={(e) =>setDescription(e.target.value)} value={description}></textarea>
          </div>
          ...
          <div>
           ...
           <div className='relative w-[300px] h-[170px]'>
            ...
            <div className='flex items-center justify-start gap-[15px]'>
              <button className='bg-[#e9e8e8] hover:bg-red-200 text-black border-1 border-black cursor-pointer px-4 py-2 rounded-md' onClick={()=>navigate("/courses")}>Cancel</button><button className='bg-black text-white px-7 py-2 rounded-md hover:bg-gray-500 cursor-pointer' onClick={handleEditCourse}>{ loading ? <ClipLoader size={30} color='white'/> : "save"}</button>
            </div>
