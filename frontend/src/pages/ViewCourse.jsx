    ...
    <div className='min-h-screen bg-gray-50 p-6'>
      <div className='max-w-6xl mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative'>
        ...
        <div>
          ...
          <p className='text-gray-700'>Beginners, aspiring developer, and professionals looking to enhance their skills in {selectedCourse?.category}.</p>
        </div>
          ...
          <div className='bg-white w-full md:w-2/5 p-6 rounded-2xl shadow-lg border border-gray-200'>
            ...
            {selectedCourse?.lectures?.map((lecture, index) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all duration-200 text-left ${
                  lecture.isPreviewFree ? "hover:bg-gray-100 cursor-pointer border-gray-300" : "cursor-not-allowed opacity-60 border-gray-200"
            ...
            <div className='bg-white w-full md:w-3/5 p-6 rounded-2xl shadow-lg border border-gray-200'>
              <div className='aspect-video w-full rounded-lg overflow-hidden mb-4 bg-black flex items-center justify-center'>
                ...
                {selectedLecture?.videoUrl ?<video src={selectedLecture.videoUrl} controls className='w-full h-full object-cover' /> : <span className='text-white text-sm'>Select a lecture to preview</span>}
              </div>
            </div>
        ...
        <div className='mt-8 border-t pt-6'>
          ...
          <div className='mb-4'>
            ...
            <textarea className='w-full border border-gray-300 rounded-lg p-2' placeholder='Write your review here...'  rows={3}/>
              <button className='bg-[black] text-white px-4 py-2 rounded hover:bg-gray-800 mt-3 cursor-pointer'>Submit Review</button>
        ...
        <div className='flex items-center gap-4 pt-4 border-t'>
          {creatorData?.photoUrl?<img src={creatorData.photoUrl} alt="" className='border-1 border-gray-300 w-16 h-16 rounded-full object-cover'/>: <img src={img} alt="" className='border-1 border-gray-600 w-16 h-16 rounded-full object-cover' />}

          <div>
            <h2 className='text-lg font-semibold'>{creatorData?.name || 'Unknown Creator'}</h2>
            <p className='text-gray-600 text-[10px]'>{creatorData?.description}</p>
        ...
        <div className='w-full transition-all duration-300 py-[20px] flex items-start justify-center lg:justify-start flex-wrap gap-6 lg:px-[80px]'>
              {
                creatorCourses?.map((course, index) => (<Card key={index} thumbnail={course.thumbnail} id={course._id} price={course.price} title={course.title} category={course.category} />
